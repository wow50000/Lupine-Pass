...
/client/proc/whitelistbunker()
	set category = "-Server-"
	set name = "Toggle Whitelist Bunker"
	if (!CONFIG_GET(flag/sql_enabled))
		to_chat(usr, span_adminnotice("The Database is not enabled!"))
		return
	...
	message_admins("[key_name_admin(usr)] has toggled the Whitelist Bunker, it is now [new_pb ? "enabled" : "disabled"].")
	if (new_pb && !SSdbcore.Connect())
		message_admins("The Database is not connected! Whitelist bunker will not work until the connection is reestablished.")
