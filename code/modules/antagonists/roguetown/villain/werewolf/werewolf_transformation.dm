...
//	src.fully_heal(FALSE) Removing this so that you don't have transformation as an easy change
	...
	if(!HAS_TRAIT(src, TRAIT_LYCAN_CURSE)) // no big stats for lycan cursed
		...
		if(src.job == "Stellari") //Lupians are adjescent to werewolves. And, since the hersir already is the WW ALPHA, it'd be understandable that the stellari are better with their curse than some random shmuck.
	...
	// This also means that people can OVERPOWER FULL WEREWOLVES, BUT they're not supposed to fight them at all (I think)
